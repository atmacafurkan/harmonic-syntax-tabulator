...
# read the derivation
df <- read_tsv("./derivations/my_derivation6.tsv") 

# enter the names 
colnames(df)[1:3] <- c("input","output","winner")

# save constraint names
constraints <- colnames(df)[4:10]

# read learned data
df_maxent <- read_tsv("./derivations/my_derivation6_output", col_names = F)
...
df %<>% mutate(optimal = ifelse(winner==1,"\\HandRight","")) %>% dplyr::select(-winner) %>% unite("eval",3:9, sep="&")
...
saveRDS(tableaus,"./to_latex/my_derivation6_latex.rds")
