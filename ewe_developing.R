...
ewe_default <- list.files(path = "./derivations/ewe_numeration/", pattern = "*.rds$", full.names = T)[4] %>% readRDS() %>% 
  mutate(across(where(is.list), ~ map_chr(.x, as.character))) %>% mutate(der = "der_ewe_def") 
ewe_matrix <- list.files(path = "./derivations/ewe_matrix/", pattern = "*.rds$", full.names = T)[4] %>% readRDS() %>% 
  mutate(across(where(is.list), ~ map_chr(.x, as.character))) %>% mutate(der = "der_ewe_mtr") 

df_eval <- rbind(eval_basic, eval_unaccusative) %>%
  rbind(ewe_default) %>% rbind(ewe_matrix) %>% mutate(input = paste(input, der)) %>% dplyr::select(-der, -mrc)

if (!dir.exists("./derivations/combined_numeration")){dir.create("./derivations/combined_numeration")}

saveRDS(df_eval, "./derivations/combined_numeration/my_eval.rds")

if (file.exists("./derivations/combined_numeration/my_optimization.rds")){
  combined_weights <- readRDS("./derivations/combined_numeration/my_optimization.rds")
  } else {
    ...
    saveRDS(combined_weights, "./derivations/combined_numeration/my_optimization.rds")
  }
...
bias <- 4
con_weights2 <- c(con_weights[1:12], con_weights[13:18]+bias)
...
df_eval %<>% dplyr::select(winner, input, operation, output, 4:21, derivation, H= harmonies, H2 = harmonies2)
...
df_ext <- df_eval[[1]] %>% dplyr::select(-derivation,-H2) %>% 
  mutate(input = factor(input, levels = unique(input))) %>%
  split(.$input) %>% lapply(prune_columns)

df_unacc <- df_eval[[2]] %>% dplyr::select(-derivation,-H2) %>% 
  mutate(input = factor(input, levels = unique(input))) %>%
  split(.$input) %>% lapply(prune_columns) 
...
tabulate_latex <- function(my_file, my_table){ 
  ...
  my_lines <- capture.output(print(xtable(my_table, caption = my_caption), include.rownames = F, caption.placement = "top",  
                                   sanitize.colnames.function = fix_text, 
                                   size = "\\scriptsize")) 
...
df_ewe_def %>% lapply(tabulate_latex, my_file = "./latex_tables/ewe_default_tables_no-mrc.txt")

df_ewe_matrix %>% lapply(tabulate_latex, my_file = "./latex_tables/ewe_matrix_tables_no-mrc.txt")
